...
constant float randomness = 0.1;
 ...
 float2 offset_rdw(float progress, float x, float theta) {
   //float phase = progress*progress + progress + theta;
   float shifty = 0.03*progress*cos(10.0*(progress+x));
   return float2(0, shifty);
 }
 float getDelta_rdw(float2 p) {
 ...
 float getDividerSize_rdw() {
   float2 rectangleSize = float2(1.0 / float2(size).x, 1.0 / float2(size).y);
   return min(rectangleSize.x, rectangleSize.y) * dividerWidth;
 }

 fragment float4 randomDownSwipeFragment(VertexOut vertexIn [[ stage_in ]],
   ...
   if(progress < pause) {
     ...
     float a = 1.0;
     if(getDelta_rdw(p) < getDividerSize_rdw()) {
       a = 1.0 - currentProg;
     }
     return mix(bgcolor, getFromColor(p,fromTexture,ratio,_fromR), a);
       ...
       if(getDelta_rdw(p) < getDividerSize_rdw()) {
             ...
             float4 c = mix(b, a, step(cp, 0.5));
             return mix(b, a, step(cp,1.0 - p.y));
